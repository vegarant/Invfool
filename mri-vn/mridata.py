...
class VnMriFilenameProducer(object):
    """Filename Producer Class for MRI data. Based on the config, it generates
    a list of all files that should be used for training and randomly shuffles
    the data and stores them in a queue. It ensures that all data is seen once
    in an epoch. For dequeing, a batch of filenames is taken out of the queue,
    defined by the config parameter `batch_size`.  All filenames of one batch
    belong to the same dataset. This allows for training with datasets of
    different image dimension, because it is ensured that all data in the batch
    have the same image dimension.
...
class VnMriReconstructionData(vn.VnBasicData):
    ...
    def get_test_data(self, dataset, patient, slice):
        ...
        return kspace, coil_sens, input0, ref, sampling_mask, norm;
