...
def discriminator(input_images, is_train=True, reuse=False):
    ...
    with tf.variable_scope("discriminator", reuse=reuse):
        ...
        net_h8 = ElementwiseLayer([net_h7, net], combine_fn=tf.add, name='h8/add')
...
def u_net_bn(x, is_train=False, reuse=False, is_refine=False):
    ...
    if reuse:
        reuse_opt = tf.AUTO_REUSE;
    else:
        resue_opt = None;
   
    with tf.variable_scope("u_net", reuse=reuse_opt):
#        tl.layers.set_name_reuse(reuse)
        ...
        if is_refine:
            ...
            out = ElementwiseLayer([out, inputs], tf.add, name='add_for_refine')
